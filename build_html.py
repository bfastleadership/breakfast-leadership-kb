...
function buildSidebar() {{
  ...
  $('ep-total-header').textContent = EPISODES.length + ' episodes';
}}
...
function renderList() {{
  ...
  $('result-count').textContent = eps.length === EPISODES.length ? `${{EPISODES.length}} episodes` : `${{eps.length}} of ${{EPISODES.length}} episodes`;
...
async function runQA() {{
  ...
  // Send compact index only (no descriptions) to keep payload small
  const compactIndex = EPISODES.map(ep => ({{
    title: ep.title,
    guest: ep.guest || null,
  ...
  const userMessage = `Episode index (${{compactIndex.length}} episodes):
